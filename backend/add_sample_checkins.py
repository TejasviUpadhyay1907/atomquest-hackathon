#!/usr/bin/env python3
"""
Add sample check-ins to populate Analytics Dashboard
"""
import sys
import os
...
def add_sample_checkins():
    db = SessionLocal()
    try:
        print("🎯 Adding Sample Check-ins for Analytics Dashboard...")
        print("=" * 60)
        ...
        if not goals:
            print("❌ No approved goals found. Please approve some goals first.")
            return
        
        print(f"✅ Found {len(goals)} approved goals")
        ...
        print(f"\n📊 Total check-ins in database: {total_checkins}")
